		...
		routingContext.vertx().executeBlocking(future -> {
			try {
				...
				if (DropShipOrderService.getOrderById(id).isEmpty()) {
					...
					String pId = "", baseId = "", colorValue = "", colorName = "", colorId = "", sizeId = "",
							sizeName = "", customData = "", campaignId = "", unitAmount = "", dsFUrl = "", dsBUrl = "",
							mFUrl = "", mBUrl = "", variantId = "", productId = "";
					int quantity = 0;
					...
					for (Map mapProduct : items) {
						...
						if(OrderProductService.getOrderProductById(pId).isEmpty()) {
							...
							rProductList = OrderProductService.updateOrderProduc(pId, id, baseId, colorValue, colorId,
									colorName, sizeId, sizeName, customData, quantity, campaignId, dsFUrl, dsBUrl, mFUrl,
									mBUrl, variantId, productId, date_update);
						}
